...
- Run `/stats` to check current usage limits (session %, weekly %, Sonnet %)
- Run `bash .Codex/scripts/check-usage.sh` to log session metrics
- If nearing session limits (session > 70% or weekly > 85%), switch to token-saving mode:
  - Use `/codex-review` instead of Codex evaluator
...
- Use the `/implement-feature` skill workflow:
  ...
  - Run `bash .Codex/scripts/verify-gate.sh` (must pass)

### 4. Evaluation
- Prefer `/codex-review --effort low` for routine reviews (saves Codex tokens)
- Use the `/evaluate` skill (Codex evaluator agent) for deep checks or when Codex is unavailable
...
watch -n 10 'cat .Codex/evidence/evidence_log.jsonl | tail -5'  # test evidence
```

## Operator controls

- **Halt**: `bash .Codex/scripts/kill-switch.sh` (or `touch AGENT_STOP`)
- **Redirect**: `bash .Codex/scripts/steer.sh "Switch to Phase 2.1"`
