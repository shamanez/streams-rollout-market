---
name: codex-review
description: Delegate code review to Codex to save Codex tokens. Uses /codex:review for standard reviews or /codex:adversarial-review for design-challenge reviews. Best for bounded, read-only review tasks.
user-invocable: true
...
Delegate code review to Codex instead of using Codex's evaluator/code-reviewer agents.
This saves Codex tokens for implementation work where Opus reasoning matters most.

## When to use this vs Codex agents

- **Use Codex**: routine reviews, lint-style checks, convention compliance, PR reviews
- **Use Codex evaluator**: when you need deep AGENTS.md firewall analysis or plan alignment checks
...
4. **If Codex fails or times out**: fall back to the Codex `code-reviewer` agent.
